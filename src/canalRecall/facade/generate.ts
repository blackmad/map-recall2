 ...
 *
 * 2.6 m per bay, from the detector's own readings over 1,375 measured façades
 * rather than from reasoning about party walls — my armchair figure was 1.9 m
 * and the measurements say otherwise. Provisional until review, but measured
 * beats reasoned, so it wins for now.
 *
 * Clamped at six: past that the plot is a warehouse or a merged pair.
 */
export const BAY_PITCH_M = 2.6;

export function bayCount(plotWidthM: number): number {
  return Math.max(1, Math.min(6, Math.round(plotWidthM / BAY_PITCH_M)));
}
...
export function bayCentres(plotWidthM: number, bays: number): number[] {
  // Enough margin for half a window plus a pier, so the outermost bay's opening
  // stays inside the plot rather than running through the party wall.
  const margin = Math.max(1.0, Math.min(1.4, plotWidthM * 0.16));

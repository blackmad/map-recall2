...
for (const building of surfaceManifest.buildings.slice(0, limit)) {
  ...
  const response = await fetch(endpoint, { signal: AbortSignal.timeout(30_000) }); if (!response.ok) { rejections.push({ buildingId: building.buildingId, reason: `catalog-http-${response.status}` }); continue; }
...
for (const tile of tiles.values()) {
  ...
  try { bytes = await readFile(file); } catch {
    const response = await fetch(tile.downloadUrl, { signal: AbortSignal.timeout(120_000) }); if (!response.ok) throw new Error(`DSM ${tile.bladnr}: HTTP ${response.status}`);

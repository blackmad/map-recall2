...
function render(){const x=items[index],old=labels[x.buildingId]||{},picked=old.selectedPanoId||x.views[0]?.panoId;q('#progress').textContent=(index+1)+' / '+items.length+' · '+Object.keys(labels).length+' reviewed';q('#app').innerHTML='<article class="card"><h2>'+esc(x.buildingId)+'</h2><div class="views">'+x.views.map((v,i)=>'<div class="view '+(v.panoId===picked?'selected':'')+'" data-pano="'+esc(v.panoId)+'"><img src="../'+esc(v.image)+'"><div class="meta">View '+(v.viewRank||i+1)+' · '+esc(v.cameraDistanceMetres)+' m · '+esc(v.observedAt)+'<br>heading '+esc(v.heading)+'° · FOV '+esc(v.fov)+'°</div></div>').join('')+'</div><div class="controls"><label>Evidence quality<select id="quality"><option value="full">Full façade visible</option><option value="partial">Useful partial façade</option><option value="unusable">No usable target view</option></select></label><label>Occlusion<select id="occlusion"><option value="none">None</option><option value="vegetation">Vegetation</option><option value="vehicle">Vehicle</option><option value="wrong-side-or-angle">Wrong side / angle</option><option value="other-building">Other building</option><option value="mixed">Mixed</option><option value="unknown">Unknown</option></select></label><label style="grid-column:1/-1">Notes<input id="notes" value="'+esc(old.notes||'')+'"></label></div></article>';q('#quality').value=old.quality||'full';q('#occlusion').value=old.occlusion||'none';document.querySelectorAll('.view').forEach(el=>el.onclick=()=>{document.querySelectorAll('.view').forEach(v=>v.classList.remove('selected'));el.classList.add('selected');if(q('#quality').value==='unusable')q('#quality').value='partial'})}

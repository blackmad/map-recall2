...
type CachedBuilding = { buildingId: string; endpoint: string; attributes: Record<string, unknown>; walls: ReturnType<typeof extractFacadeWallPlanes> };
...
const pending = buildingIds.filter((buildingId) => !byBuilding.has(buildingId));
for (let offset = 0; offset < pending.length; offset += 4) {
  const fetched = await Promise.all(pending.slice(offset, offset + 4).map(async (buildingId): Promise<CachedBuilding | Rejection> => {
    ...
    const walls = extractFacadeWallPlanes(source);
    const attributes = source.feature?.CityObjects?.[`NL.IMBAG.Pand.${bagId}`]?.attributes || {};
    return { buildingId, endpoint, attributes, walls };
  }));
  ...
  const manifest = { schemaVersion: 1, generatedAt: new Date().toISOString(), source: { name: '3DBAG API', crs: 'EPSG:7415', geometry: 'LoD2.2 semantic exterior WallSurface' }, buildings: [...byBuilding.values()], rejections: [...rejections.values()] };

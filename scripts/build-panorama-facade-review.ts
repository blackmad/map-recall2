...
<header><strong>Amsterdam façade review</strong><button id="prev" class="muted">← Previous</button><button id="next" class="primary">Save & next →</button><button id="import" class="muted">Import JSON</button><button id="export" class="muted">Export JSON</button><input id="import-file" type="file" accept="application/json" hidden><span id="progress" class="progress"></span></header><main id="app"></main><script>
...
const checkpoint = async () => {
  ...
  await writeFile(path.join(outputDirectory, 'machine-labels.json'), JSON.stringify({ schemaVersion: 2, generatedAt, model: model || null, labels: items.filter(item => item.proposal).map(item => ({ buildingId: item.buildingId, bagId: item.bagId, osmId: item.osmId, panoId: item.panoId, observedAt: item.observedAt, source: 'model-panorama', reviewStatus: 'machine-proposal', acceptedForNow: false, ...item.proposal as object })) }, null, 2));
};
...
await writeFile(path.join(outputDirectory, 'machine-labels.json'), JSON.stringify({ schemaVersion: 2, generatedAt: manifest.generatedAt, model: model || null, labels: items.filter(item => item.proposal).map(item => ({ buildingId: item.buildingId, bagId: item.bagId, osmId: item.osmId, panoId: item.panoId, observedAt: item.observedAt, source: 'model-panorama', reviewStatus: 'machine-proposal', acceptedForNow: false, ...item.proposal as object })) }, null, 2));

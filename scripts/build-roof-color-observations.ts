...
const selectionCentre: Point = bbox ? [(bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 2] : dam;
const candidates = source.features.filter(x => { const p = centre(x); return !processed.has(String(x.properties.buildingId)) && (!bbox || p[0] >= bbox[0] && p[0] <= bbox[2] && p[1] >= bbox[1] && p[1] <= bbox[3]); }).sort((a, b) => { const pa = centre(a), pb = centre(b); return Math.hypot(pa[0] - selectionCentre[0], pa[1] - selectionCentre[1]) - Math.hypot(pb[0] - selectionCentre[0], pb[1] - selectionCentre[1]); }).slice(0, limit);
